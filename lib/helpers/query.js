...
class Query {
  ...
  get () {
    ...
    if (!where['$obj'][this.key] && !Object.keys(where['$obj']).length) {
  ...
  post (data) {
    ...
    if (this.target) {
      if (this.skipKeys.length) {
        this.target.skipKeys = this.skipKeys
      }
      if (this.deferKeys.length) {
        this.target.deferKeys = this.deferKeys
      }
  ...
  get deferKeys () {
    return this._deferKeys || null
  }

  set deferKeys (keys) {
    this._deferKeys = keys
  }
